...
- **Docs theme flicker:** all Pages (`index`, `roadmap`, `release`) now share one theme key (`nssm-theme`) and apply it via a blocking `<head>` script before first paint — no flash and no theme "jump" when navigating between pages. Added a smooth light/dark transition.
- **Docs layout declutter:** roadmap and release pages use more breathing room (wider section spacing, larger headings), unified green/teal brand mark with the landing page, and toned-down decorative watermarks.
...
- **GitHub Pages single-page hub:** `docs/index.html` is now a tabbed single-page app (**Overview / Release / Roadmap / Download**) with hash routing — no page reloads or theme jumps. `docs/app.js` reads the existing `release-data.js` and `roadmap-data.js` and renders both inline; `docs/app.css` holds the shared styling. The **Download** tab fetches the latest GitHub release via the API and lists assets (zip + SHA-256) with a direct-link fallback. Published via `.github/workflows/pages.yml`. Standalone `docs/release/<x.y.z>/` and `docs/roadmap/` pages remain as the data source and for deep links. The `docs-update-on-change` rule lists `docs/index.html` as a maintained target.
- **Docs SPA parse error:** fixed a mismatched quote in `docs/app.js` (single-quoted HTML string closed with `"`) that broke the entire hub with `SyntaxError` at line 124.
- **Release tab polish:** restyled the **Release** view in `docs/app.css` — accent bars on section headings, a gradient version badge, calmer teal CLI command cards with a `$` prompt and hover lift, registry parameters grouped into a responsive card grid, circular check badges on the feature list, and the source-file table wrapped in a panel.
- **Header balance:** the top nav (`docs/app.css`) now uses a 3-column grid so the tab bar is centered (logo left, tabs center, GitHub/theme icons right) instead of sitting next to the logo with an empty gap on the right.
- **Download tab:** now shows the real **SHA-256** hash inline with a copy button (baked into `docs/release/2.25/release-data.js` as `download.sha256`, so it survives GitHub API rate-limits/offline), adds a **Quick install** snippet card, and the rate-limited/offline fallback lists direct download links + checksum instead of a single bare button.
