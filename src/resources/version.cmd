@rem Default version. Prefer the repo-root VERSION file (passed in via
@rem NSSM_VERSION_FALLBACK by build.cmd); otherwise 0.0.
if defined NSSM_VERSION_FALLBACK (set description=%NSSM_VERSION_FALLBACK%-0-release) else set description=0.0-0-prerelease
@rem Get canonical version from git tags, eg v2.21-24-g2c60e53 (overrides the file).
for /f %%v in ('git describe --tags --long 2^>NUL') do set description=%%v
